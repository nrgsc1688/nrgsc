...
extern crate nrgsc;
extern crate nrgsc_object_base;
extern crate nrgsc_wallet_base;
extern crate serde;
...
use nrgsc::cache::nrgsc_CACHE;
use nrgsc::error::Result;
use nrgsc::joint::{Joint, JointSequence};
use nrgsc::network::wallet::WalletConn;
use nrgsc::statistics::{LastConnStat, StatsPerPeriod};
use nrgsc::try_go;
use nrgsc::validation;
use nrgsc::wallet_info::{WalletInfo, MY_WALLET};
...
fn init(verbosity: u64) -> Result<()> {
    ...
    let workers = nrgsc::config::get_worker_thread_num();
    may::config()
...
fn connect_to_remote(peers: &[String]) -> Result<Arc<WalletConn>> {
    for peer in peers {
        match nrgsc::network::wallet::create_outbound_conn(&peer) {
            Err(e) => {
...
fn send_payment(
    ...
    let text_message = match text {
        Some(msg) => Some(nrgsc::composer::create_text_message(msg)?),
        None => None,
    };
    ...
    let outputs = address_amount
        .iter()
        .map(|(address, amount)| nrgsc::spec::Output {
    ...
    let inputs: nrgsc::light::InputsResponse = ws.get_inputs_from_hub(
    ...
    let joint = nrgsc::composer::compose_joint(compose_info, wallet_info)?;
    ...
    println!(
        "DATE  : {}",
        Local
            .timestamp_millis(nrgsc::time::now() as i64)
            .naive_local()
    );
...
fn verify_joints(joints: Vec<Joint>, last_mci: usize) -> Result<()> {
    ...
    for joint in joints {
        try_go!(move || {
            ...
            let cached_joint = match nrgsc_CACHE.add_new_joint(joint, None) {
                Ok(j) => j,
                Err(e) => {
    ...
    while !sem.wait_timeout(Duration::from_secs(1)) {
        println!("current mci={:?}", nrgsc::main_chain::get_last_stable_mci());
    }

    let dur = now.elapsed();
    println!("\ntotal mci={:?}", nrgsc::main_chain::get_last_stable_mci());
...
fn register_event_handlers(last_mci: usize, sem: Arc<Semphore>) {
    use nrgsc::main_chain::MciStableEvent;
    use nrgsc::utils::event::Event;
...
fn main() -> Result<()> {
    ...
    if let Some(init_arg) = m.subcommand_matches("init") {
        ...
        // ::std::fs::remove_file(nrgsc::config::get_database_path(true)).ok();
        return Ok(());
    }
