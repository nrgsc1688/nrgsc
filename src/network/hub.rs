...
use cache::{JointData, nrgsc_CACHE};
use catchup;
use composer::*;
use config;
use error::Result;
...
use nrgsc_object_base::object_hash;
...
impl HubConn {
    ...
    fn on_get_joints_info(&self, _param: Value) -> Result<Value> {
        Ok(json!(light::NumOfUnit {
            valid_unit: nrgsc_CACHE.get_num_of_normal_joints(),
            known_bad: nrgsc_CACHE.get_num_of_bad_joints(),
            temp_bad: nrgsc_CACHE.get_num_of_temp_bad_joints(),
            unhandled: nrgsc_CACHE.get_num_of_unhandled_joints(),
    ...
    fn on_get_light_props(&self, param: Value) -> Result<Value> {
        ...
        let light_props = light::LightProps {
            ...
            has_definition: nrgsc_CACHE.get_definition(&address).is_some(),
        };
    ...
    fn on_subscribe(&self, param: Value) -> Result<Value> {
        ...
        try_go!(move || -> Result<()> {
            ...
            if let Some(last_mci) = last_mci {
                ...
                // send genesis unit
                let genesis = nrgsc_CACHE.get_joint(&::spec::GENESIS_UNIT)?.read()?;
    ...
    fn on_get_joint(&self, param: Value) -> Result<Value> {
        ...
        match nrgsc_CACHE.get_joint(&unit).and_then(|j| j.read()) {
            Ok(joint) => {
    ...
    fn on_get_free_joints(&self, _param: Value) -> Result<Value> {
        match nrgsc_CACHE.get_good_free_joints() {
            Ok(mut joints) => {
    ...
    fn on_get_missing_joints(&self, _param: Value) -> Result<Value> {
        let joints = nrgsc_CACHE.get_missing_joints();
        Ok(json!(joints))
    }
    ...
    fn on_free_joint_list(&self, param: Value) -> Result<()> {
        ...
        for unit in free_units {
            ...
            if nrgsc_CACHE.check_new_joint(&unit).is_ok() {
    ...
    fn on_get_network_info(&self, _param: Value) -> Result<Value> {
        ...
        let total_units = nrgsc_CACHE.get_joints_len();
    ...
    fn on_get_bad_joints(&self, _param: Value) -> Result<Value> {
        Ok(serde_json::to_value(nrgsc_CACHE.get_bad_joints())?)
    }

    fn on_get_temp_bad_joints(&self, _param: Value) -> Result<Value> {
        Ok(serde_json::to_value(nrgsc_CACHE.get_temp_bad_joints())?)
    }

    fn on_get_children(&self, param: Value) -> Result<Value> {
        let unit: String = serde_json::from_value(param)?;

        let joint = nrgsc_CACHE.get_joint(&unit)?.read()?;
        let children = joint
            .children
            .iter()
...
impl HubConn {
    fn handle_online_joint(&self, joint: Joint, is_post: bool) -> Result<()> {
        ...
        let cached_joint = match nrgsc_CACHE.add_new_joint(joint, Some(self.get_peer_id())) {
            Ok(j) => j,
            Err(e) => {
        ...
        if let Some(ball) = &joint_data.ball {
            if !nrgsc_CACHE.is_ball_in_hash_tree(ball) {
                ...
                go!(move || {
                    ...
                    nrgsc_CACHE.clear_hash_tree_ball();
                });
                return Ok(());
            }
        } else {
    ...
    fn request_new_missing_joints<'a>(
        ...
        for unit in units {
            ...
            if let Err(e) = nrgsc_CACHE.check_new_joint(unit) {
    ...
    fn send_joints_since_mci(&self, mci: Level) -> Result<()> {
        ...
        for joint in nrgsc_CACHE.get_joints_by_mci(last_stable_mci)? {
    ...
    fn send_free_joints(&self) -> Result<()> {
        let joints = nrgsc_CACHE.get_good_free_joints()?;
        for joint in joints {
...
pub fn broadcast_free_joint_list() {
    if let Ok(free_joints) = nrgsc_CACHE.get_good_free_joints() {
...
pub fn purge_junk_unhandled_joints(timeout: u64) {
    ...
    nrgsc_CACHE.purge_old_unhandled_joints(now, timeout);
}

/// remove those long time temp-bad free joints
pub fn purge_temp_bad_free_joints(timeout: u64) -> Result<()> {
    let now = crate::time::now();
    nrgsc_CACHE.purge_old_temp_bad_free_joints(now, timeout)
}
...
pub fn re_request_lost_joints() -> Result<()> {
    ...
    let units = nrgsc_CACHE.get_missing_joints();
    if units.is_empty() {
        return Ok(());
    }
...
pub fn notify_watchers_about_stable_joints(mci: Level) -> Result<()> {
    ...
    let last_ball_mci = nrgsc_CACHE.get_last_ball_mci_of_mci(mci)?;
    let prev_last_ball_mci = nrgsc_CACHE.get_last_ball_mci_of_mci((mci.value() - 1).into())?;
...
fn start_catchup(ws: Arc<HubConn>) -> Result<()> {
    ...
    assert_eq!(nrgsc_CACHE.get_hash_tree_ball_len(), 0);
    ...
    for batch in catchup_chain_balls.windows(2) {
        ...
        ::utils::wait_cond(Some(Duration::from_secs(10)), || {
            nrgsc_CACHE.get_hash_tree_ball_len() < 1000
        })
    ...
    ::utils::wait_cond(Some(Duration::from_secs(10)), || {
        nrgsc_CACHE.get_hash_tree_ball_len() == 0
    })
