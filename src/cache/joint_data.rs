...
use cache::{CachedJoint, nrgsc_CACHE};
use error::Result;
    ...
    pub fn get_max_stable_unit(&self) -> Result<RcuReader<JointData>> {
        if self.is_min_wl_increased() {
            if !self.max_stable_unit.is_completed() {
                let joint = nrgsc_CACHE.get_joint(&self.unit.unit)?.read()?;
...
impl JointData {
    ...
    fn calc_best_parent(&self) -> Result<()> {
        ...
        if let Some(best_parent) = find_best_joint(self.parents.iter())? {
            let best_parent = nrgsc_CACHE.get_joint(&best_parent.unit.unit)?;
    ...
    pub fn get_last_ball_joint(&self) -> Result<RcuReader<JointData>> {
        ...
        match self.unit.last_ball_unit {
            Some(ref unit) => nrgsc_CACHE.get_joint(unit)?.read(),
            None => nrgsc_CACHE.get_joint(&::spec::GENESIS_UNIT)?.read(),
        }
    }
...
impl LoadFromKv<String> for JointData {
    fn load_from_kv<T: ::std::borrow::Borrow<String>>(key: &T) -> Result<Self> {
        ...
        let parents = joint
            ...
            .parent_units
            .iter()
            .map(|key| nrgsc_CACHE.get_joint_or_none(key))
            .collect();
        ...
        best_parent.append(nrgsc_CACHE.get_joint_or_none(&props.best_parent_unit));
