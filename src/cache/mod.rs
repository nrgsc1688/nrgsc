...
lazy_static! {
    pub static ref nrgsc_CACHE: nrgscCache = nrgscCache::default();
}
...
impl CachedJoint {
    pub fn save_to_db_async(&self) -> Result<()> {
...
#[derive(Default)]
pub struct nrgscCache {
    // keep all joints here, the source of every thing
    joints: RwLock<cache_impl::nrgscCacheInner>,
...
impl nrgscCache {
    ...
    pub fn get_joints_by_mci(&self, mci: Level) -> Result<Vec<CachedJoint>> {
        let joint = match self.get_mc_unit_hash(mci)? {
            ...
            Some(unit) => nrgsc_CACHE.get_joint(&unit)?,
        };
    ...
    /// only not good free joints can be purged, use the func carefully !!!
    pub fn purge_free_joint(&self, unit: &str) -> Result<()> {
        self.joints.write().unwrap().purge_free_joint(unit)
    }
