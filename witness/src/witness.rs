...
use nrgsc::business::BUSINESS_CACHE;
use nrgsc::cache::{CachedJoint, JointData, nrgsc_CACHE};
use nrgsc::error::Result;
use nrgsc::joint::JointSequence;
use nrgsc::joint::Level;
use nrgsc::my_witness::MY_WITNESSES;
use nrgsc::wallet_info::MY_WALLET;
use nrgsc_wallet_base::Base64KeyExt;
...
lazy_static! {
    ...
    static ref SELF_LEVEL: AtomicIsize = AtomicIsize::new(1 - nrgsc::config::MAJORITY_OF_WITNESSES as isize);
...
fn is_need_witnessing() -> Result<(bool)> {
    let free_joints = nrgsc_CACHE.get_all_free_joints();
    ...
    let best_joint = nrgsc::main_chain::find_best_joint(free_joints.iter())?
...
fn check_self_level(free_joints: &[CachedJoint]) -> Result<(bool)> {
    ...
    for unit in free_joints {
        ...
        if level.value() as isize - self_level >= nrgsc::config::MAJORITY_OF_WITNESSES as isize - 2 {
            return Ok(true);
        }
    }
...
fn is_relative_stable(mut best_free_parent: RcuReader<JointData>) -> Result<(bool, bool)> {
    ...
    while !(best_free_parent.is_stable() || best_free_parent.unit.is_genesis_unit()) {
        ...
        if diff_witnesses.len() >= nrgsc::config::MAJORITY_OF_WITNESSES - 1 {
            break;
        }
...
fn is_need_witness_normal_joint(
    ...
    if let Some(joint) = get_unstable_latest_normal_joint(free_joints)? {
        if let Some(joint) = find_best_include_mc_joint(best_joint.clone(), joint)? {
            ...
            if SELF_TIME.load(Ordering::Relaxed) + 10_000 > nrgsc::time::now() as usize
                && nrgsc::main_chain::is_stable_to_joint(&joint, &best_joint)?
            {
                return Ok(false);
            }
        }
...
fn compose_and_normalize() -> Result<()> {
    let nrgsc::composer::ParentsAndLastBall {
        parents,
        last_ball,
        last_ball_unit,
    } = nrgsc::composer::pick_parents_and_last_ball(&MY_WALLET._00_address)?;
    ...
    let light_props = nrgsc::light::LightProps {
        last_ball,
        last_ball_unit,
        parent_units: parents,
        witness_list_unit: nrgsc::spec::GENESIS_UNIT.to_string(),
        has_definition: nrgsc_CACHE.get_definition(&MY_WALLET._00_address).is_some(),
    };

    let compose_info = nrgsc::composer::ComposeInfo {
        paid_address: MY_WALLET._00_address.clone(),
        change_address: MY_WALLET._00_address.clone(),
        outputs: Vec::new(),
        inputs: nrgsc::light::InputsResponse { inputs, amount },
    ...
    // if nrgsc::config::get_need_post_timestamp() {
    //     let time_stamp = TimeStamp {
    //         timestamp: nrgsc::time::now() / 1_000,
    //     };
    //     let data_feed_msg = nrgsc::spec::Message {
    //         app: "data_feed".to_string(),
    //         payload_location: "inline".to_string(),
    //         payload_hash: object_hash::get_base64_hash(&time_stamp)?,
    //         payload: Some(nrgsc::spec::Payload::Other(serde_json::to_value(
    //             time_stamp,
    //         )?)),
    //         payload_uri: None,
    ...
    let joint = nrgsc::composer::compose_joint(compose_info, &*MY_WALLET)?;

    let cached_joint = nrgsc_CACHE.add_new_joint(joint, None)?;

    let joint_data = cached_joint.read()?;
    joint_data.set_is_post(true);
    nrgsc::validation::validate_ready_joint(cached_joint)?;
    let sequence = joint_data.get_sequence();
    if sequence != JointSequence::Good {
        // purge the bad composed joint
        nrgsc_CACHE.purge_free_joint(&joint_data.unit.unit)?;
        bail!(
    ...
    SELF_TIME.store(nrgsc::time::now() as usize, Ordering::Relaxed);
    ...
    if let Some(ws) = nrgsc::network::hub::WSS.get_next_peer() {
